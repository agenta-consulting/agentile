...
Specs can declare `depends_on: [slug, …]` in their frontmatter, listing other specs that must ship before this one can be claimed. When a spec ships it moves to `specs/done/`, keeping the active numbered list clean while remaining resolvable as a fulfilled dependency. A spec that won't ship is dropped with `/ag-abandon`, which records the reason and moves it to `specs/abandoned/`, walking the dependency chain so nothing downstream is left silently blocked. The whole backlog lives under one configurable directory (`docs/agentile/` by default).
