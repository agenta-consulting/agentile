...
Design spec. Status: **proposed** 2026-09-03. Extends the customisation-and-concurrency contract (`agentile-customisation-and-concurrency.md`) and the backlog layout (`agentile-backlog-layout-and-abandon.md`). Nothing here changes the loop's stages; it changes *where the loop runs*, *who may act at each stage*, and *where the backlog lives*.
