---
name: lal-capture
description: Append a one-line stub to the Lean Agentic Loop inbox with today's date. Instant capture — no questions, no work, safe to run mid-build. Trigger phrases include "/lal-capture", "capture this idea", "drop a stub", "add to the inbox", "note this down for later".
...
- **Do not start work, plan, or shape.** That is what `/lal-shape` is for.
...
2. Find the inbox path: read `.lal/config.md` for the **Inbox** path under "## Paths". Default to `inbox.md` at repo root if `.lal/config.md` is absent.
...
4. Append a new line at the end of the file, under the `# Inbox` heading:
   ...
   If the file does not exist yet, tell the user to run `/lal-init` first (do not silently create a bare inbox — the project may not be initialised).
