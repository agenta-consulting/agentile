...
is enabled; fallback `"${CLAUDE_PLUGIN_ROOT}/bin/ag-store"`.) It returns a JSON array of
the transitive set of active specs whose `depends_on` reaches the target, nearest first
(empty if none). These are the specs that will be left waiting if the target is
abandoned.
