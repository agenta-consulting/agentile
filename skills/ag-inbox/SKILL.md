...
3. List the stubs: `ag-store inbox_list --dir "<Agentile directory>" --store "<store>"` (bare command; fallback `"${CLAUDE_PLUGIN_ROOT}/bin/ag-store"`). A non-zero exit means the inbox doesn't exist — tell the user to run `/ag-init` first. The output is a JSON array of `{id, title, type, text, captured_at, captured_by}` — parse it directly (`title` and `type` are null on the `local` store, and on stubs captured before those fields existed).
4. Present each stub numbered by its `id`, exactly as captured — including its capture date, and its `captured_by` when the store records one (a shared store; `local` leaves this blank). Lead the line with its `title` when it has one, tag it with its `type` when that is anything but `feature`, then the full text; the title is a label for scanning, never a replacement for the stub. Do not reword, triage, or summarise them.
