---
name: lal-shape
description: Promote a stub from the Lean Agentic Loop inbox to a Ready spec through a conversation — interview the user against the project's Definition of Ready, run the Business Value × Technical Certainty triage, then write the spec and remove the stub. Non-coding. Trigger phrases include "/lal-shape", "shape this stub", "shape an inbox item", "turn this into a spec", "make this Ready".
...
- Read `.lal/shaping.md` — this is **this project's Definition of Ready**: the exact questions a stub must answer, plus any house additions. Drive the interview against *this* list, not a generic one.
- Read `.lal/config.md` for the inbox/specs paths and the two-axis triage table.
...
- Work through every required item in `.lal/shaping.md` — typically problem/who/why-now, acceptance criteria, edge cases and failure paths, scope boundary, affected areas, and open questions — plus any house additions.
- Prefer concrete examples over abstractions. Push back gently on vague acceptance criteria.

## Step 4 — Triage

- Estimate **Business Value** and **Technical Certainty** (guidance in `.lal/config.md`).
...
- **Graduate to a spec** — write `specs/<slug>.md` using `.lal/spec-template.md`, filling every field from the interview, and set `route`, `business_value`, `technical_certainty`. Then remove the stub line from the inbox.
...
Confirm what you wrote (path), the recommended route, and the next step (usually `/lal-plan specs/<slug>.md`). Do not start building.
